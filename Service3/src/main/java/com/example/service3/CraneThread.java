...
public class CraneThread extends Thread
{
    ...
    private void getStat(int localShipIndex)
    {
        ...
        if (shipsInput.get(localShipIndex).getDate().getTimeInMillis() < date.getTime())
        {
            statInfo[0] += date.getTime() - shipsInput.get(localShipIndex).getDate().getTimeInMillis();
            serviceInfoArrayList.get(localShipIndex + shift).waitingTime.setTime(date.getTime()
              - shipsInput.get(localShipIndex).getDate().getTimeInMillis());
        ...
        } else
        {
            serviceInfoArrayList.get(localShipIndex
              + shift).beginningUnloadingTime.setTime(shipsInput.get(localShipIndex).getDate().getTimeInMillis());
            serviceInfoArrayList.get(localShipIndex + shift).waitingTime.setTime(0);
        }
        serviceInfoArrayList.get(localShipIndex
          + shift).comingTime.setTime(shipsInput.get(localShipIndex).getDate().getTimeInMillis());
    }
    ...
    public void run()
    {
        while (true)
        {
            ...
            if (isFirstCrane)
            {
                ...
                while (date.getTime() > shipsInput.get(j).getDate().getTimeInMillis())  // добавляем длину очереди
                {
                    if (j != localShipIndex)
                    {
                ...
                int fineHours = (int) (((date.getTime() - shipsInput.get(localShipIndex).getDate().getTimeInMillis())
