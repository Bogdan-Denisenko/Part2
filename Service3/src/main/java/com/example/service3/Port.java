...
public class Port
{
    ...
    public void calculateOptimalNumberOfCranes(String schedule)
    {
        ...
        ships.sort(new Comparator<Ship>() //сортируем корабли по дате прибытия
        {
            ...
            {
                return Long.compare(lhs.getDate().getTimeInMillis(), rhs.getDate().getTimeInMillis());
            }
        });
        ...
        {
            if (ships.get(i).getDate().get(Calendar.MONTH) == 2)
            {
                         ...
                         double craneSpeed, int shift, int[] delayTime) throws InterruptedException
    {
        ...
        CraneThread.date = new Date(shipsInput.get(0).getDate().getTimeInMillis());
        for (int i = 0; i < craneCount; i++)
        {
        ...
        while (CraneThread.countNotWorkingThreads.get() != craneCount)
        {
                if (shipIndex < shipsInput.size()) {
                    if (CraneThread.date.getTime() >= shipsInput.get(shipIndex).getDate().getTimeInMillis()) {
                        sem.release(2);
                        shipIndex++;
                    }
                }
