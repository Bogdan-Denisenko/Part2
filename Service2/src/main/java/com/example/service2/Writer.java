...
import com.example.service3.CommonStatistics;
import com.example.service3.ServiceInfo;
...
public class Writer
{
    ...
    @GetMapping("/tableTwo")
    public String shipsTableTwo(@RequestParam("name") String name) throws IOException {
        File file = new File(System.getProperty("user.dir") + "/" + name);
        if (!file.exists()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "File with this name is not exist");
        }
    ...
    public static void writeResult(@RequestBody CommonStatistics commonStatistics)
    {
        ...
        try {
            ...
            for (ServiceInfo info : serviceInfo) {
                ...
                writer.name("coming time").value(String.valueOf(info.comingTime));
                writer.name("waiting time").value(String.valueOf(info.waitingTime));
                writer.name("beginning unloading time").value(String.valueOf(info.beginningUnloadingTime));
                writer.name("unloading time").value(String.valueOf(info.unloadingTime));
                writer.endObject();
            }
            writer.endArray();
            writer.endObject();
